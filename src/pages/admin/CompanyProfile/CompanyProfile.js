...
const CompanyProfile = () => {
  ...
  const { isLoading,
    companyData,
    branchInfo,
    cnBranch } = useSelector(state => state.company);
      ...
      <div className="row layout-spacing pt-4">
        {!isLoading ?
          <>
            ...
            {country && Object.keys(cnBranch).length > 0
              && typeof cnBranch?.data !== 'undefined'
              && Object.keys(cnBranch?.data)?.length > 0
              && typeof cnBranch?.data?.company_info !== 'undefined'
              && cnBranch?.data?.company_info !== null ?
              <CompanyDescription
                info={cnBranch?.data?.company_info}
                data={cnBranch?.data?.branches}
                bInfo={branchInfo}
                country={country}
                branchId={branchId} />
              : Object.keys(companyData)?.length > 0
            ...
            {country && Object.keys(branchInfo).length > 0
              ...
              : country && Object.keys(cnBranch).length > 0
                && typeof cnBranch?.data !== 'undefined'
                && Object.keys(cnBranch?.data)?.length > 0
                && typeof cnBranch?.data?.team_members !== 'undefined' ?
                <TeamMembers info={companyData?.data?.company_info}
                  data={cnBranch?.data?.team_members} />
                : Object?.keys(companyData)?.length > 0
                && typeof companyData?.data !== 'undefined'
                && Object.keys(companyData?.data)?.length > 0
