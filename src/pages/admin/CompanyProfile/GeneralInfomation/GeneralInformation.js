...
const GeneralInformation = ({
  ...
  useEffect(() => {
    if (branchId) {
      ...
      if (typeof cnBranch !== 'undefined'
        && Object.keys(cnBranch).length > 0
        && Object.keys(cnBranch.data).length > 0) {
        setAddress(cnBranch?.data?.company_info);
      }
    } else {
      setAddress(false);
    }
  }, [branchId, country, cnBranch]);
    ...
    <>
      <div className="col-xl-4 col-lg-4 col-md-12 mb-4">
        <div className="profile-left">
          ...
          <div className="col-12 d-flex justify-content-center info-area" style={{ height: "40px", fontSize: "16px", fontWeight: "700" }}>
            {data.company_name}
          </div>
        ...
        <div className="profile-info">
          <div className='d-flex flex-row'>
            ...
            <div className='col-2 d-flex justify-content-end'>
              <div className='d-flex flex-row'>
                ...
                <div>
                  <h6 style={{ textTransform: "capitalize", color: "#000", fontSize: "10px" }}>
                    {data?.status}
                  </h6>
                </div>
              </div>
            ...
            <div className="d-flex flex-md-row flex-column col-md-6 col-12 single-profile-info gap-2">
              ...
              {!lod && country &&
                <div className='col-md-6 col-12 mt-md-0 mt-3'>
                  <h6>Branch</h6>
                  {Object.keys(cnBranch)?.length > 0
                    && Object.keys(cnBranch?.data)?.length
                    && typeof cnBranch?.data?.branches !== 'undefined'
                    && cnBranch?.data?.branches?.length > 0
                    ?
                    <Form.Select aria-label="Default select example" size="sm"
                      onChange={(e) => setBranchId(e.target.value)}
                      value={branchId}>
                      <option value="">Select Branch</option>
                      {cnBranch.data.branches.map((item, index) => {
          ...
          <div className="d-flex flex-md-row flex-column">
            <div className="col-md-5 col-12 single-profile-info d-flex flex-column ">
              ...
              <div>
                <a className='websiteLink' onClick={() => validateUrl(data.website)} style={{ color: "darkgrey", cursor: "pointer", fontSize: "12px" }}>{data.website}</a>
              </div>
              <div className="d-flex flex-row ">
