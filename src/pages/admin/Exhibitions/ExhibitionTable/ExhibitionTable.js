...
const ExhibitionTable = () => {
  ...
  useEffect(() => {
    const intv = setInterval(() => {
      ...
      if(SCD && typeof SCD.company != 'undefined' ||  SCD && typeof SCD.country !== 'undefined')
      {
        dispatch(getExhibitionListID(SCD))
      }
      else
      {
        dispatch(getExhibitionListID({ event_id: id }))
      ...
      exhibitionChat.data.messages.map(item => {
        ...
        if (type !== '') {
          msg.push({
            ...
            receiver_date: item?.receiver_date,
            type: type
          });
        }
      })
  ...
  return (
    <>
      {typeof id !== 'undefined' &&
        <>
          <div className="col-xl-12 col-lg-12 align-self-center w-100 mb-3 mt-3">
            <Link to="/exhibitions">
                      ...
                      chatMsg.map((item, index) => {
                        return (<div key={`c${index}`}>
                          ...
                          <div className={`d-flex justify-content-${item.type === 'sender' ? "end" : "start"}`}>
                            <p className="small time-text-muted">{item.type === 'sender' ? item.sender_date : item.receiver_date}</p>
                          </div>
                        </div>
                        )
                      })}
